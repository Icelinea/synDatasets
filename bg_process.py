...
def label_deduplicate(name, threshold=0.1):
    ...
    # 使用 LLM + 人工填充缺失标签
    setupPrompt = "你是一位心理健康领域的专家学者，受邀参与一项关于共情对话主题的数据分类工作。你会得到病患的背景描述数据，请你通过分析病患背景描述数据来判断病患的核心病症。请注意，病患的核心病症只有以下 9 种类别：{}，其中它们对应的详细解释如下：{}。你的回答只能是你认为的 9 种核心病症中的和所提供的病患背景数据最接近的那一种，注意你的回答只能是核心病症 9 种类别种的专有名词，只能有四个字，不能包含任何其他文字或者符号。".format('，'.join(topics), "；".join(dialogueTopics))
