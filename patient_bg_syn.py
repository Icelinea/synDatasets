...
setupPrompt = "你是一名心理健康领域的专家，以下是你需要理解的在与病患对话时的核心病症的对话主题种类，其中 [] 内的是核心病症主题的专有名词，后面接着的是该专有名词的解释，你所进行的所有的心理健康对话的核心病症的主题都可以被归类到以下的情况当中：{}。你生成的消息一定要按照以下的 json 格式输出，其中 () 内表示你需要生成的内容，生成时请去除 () 并且不要生成多余的回复内容或者多余的标点符号，外层是列表符号 []，内层是字典符号 {{}}，具体 json 格式如下：[{{\"年龄\": (), \"性别\": (), \"主题\": (), \"背景\": ()}}]。注意其中年龄为人类的正常年龄，只能是数字，不能小于 0 岁；性别只有'男'和'女'两种；主题你只能选择\"躯体症状\"；背景是与你对话的病患所提供的病情背景信息，需要由你根据所选择的主题的专有名词对应的核心病症解释进行生成，字数不能少于 20 字且不能多于 100 字，并且请不要生成涉及真实人员隐私的信息，例如人名、地名、建筑名、身份证等隐私信息。".format("；".join(dialogueTopics))
...
if __name__ == '__main__':
    background_synthesis("patient", 120, 1, 10, "./data/CPsyCounR/CPsyCounR.json", "./data/PatientBackground/raw/bg-somatic-pad2.json")