...
# 患者背景知识生成
# 额外知识 + 输出格式
setupPrompt = "你是一名心理健康领域的专家，以下是你需要理解的在与病患对话时的核心病症的对话主题种类，你所进行的所有的心理健康对话的核心病症的主题都可以被归类到以下的情况当中：{}。\n你生成的消息一定要按照以下的 json 格式输出，其中 () 内表示你需要生成的内容，生成时请去除 () 并且不要生成多余的空格或者不符合 json 格式或者要求的标点符号，外层是 python 的列表符号 []，内层是 python 的字典符号 \{\}，具体 json 格式如下：[{'年龄': (), '性别': (), '主题': (), '背景': ()}]。注意其中年龄为人类的正常年龄，不能小于 0 岁；性别只有'男'和'女'两种；主题是心理健康对话的核心病症的主题的其中一个，需要由你判断所生成的患者背景信息最符合的病症主题，并且只能在提供的情况内选择，有且只有一种主题；背景是与你对话的病患所提供的病情背景信息，需要由你生成。".format(dialogueTopicsEn2Cn)
...
if __name__ == '__main__':
    background_synthesis("patient", 3, 3, 3, "./data/CPsyCounR/CPsyCounR.json", "./data/PatientBackground")